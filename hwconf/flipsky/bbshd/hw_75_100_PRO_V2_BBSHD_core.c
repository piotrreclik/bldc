...
#include "../../luna/bbshd/luna_display_serial.h"
...
void hw_init_gpio(void) {
	...
	motor_has_PTC_sensor = false;// hw_bbshd_has_PTC_sensor();
...
void hw_update_speed_sensor(void) {
	...
	if(sensor_state == 0 && sensor_state_old == 1 ) {
		...
		if (revolution_duration > 0.05) {	//ignore periods <50ms
			...
			wheel_rpm = revolution_duration > 10.0 ? 0.0 : (60.0 / revolution_duration);
			UTILS_LP_FAST(wheel_rpm_filtered, (float)wheel_rpm, revolution_duration > 1 ? 0.9 : 0.7);
		...
		// After 4 seconds without sensor signal, set RPM as zero
		if ( (current_time - last_sensor_event_time) > 5.0) {
