...
void setup_mode(uint8_t assist_code) {
	if (ebike_configure_mode) {
		switch (assist_code) {
			case PAS_LEVEL_0: app_ebike_set_mode(EBIKE_MODE_LEGAL); break;
			case PAS_LEVEL_6: app_ebike_set_mode(EBIKE_MODE_UNRESTRICTED); break;
			case PAS_LEVEL_9: app_ebike_set_mode(EBIKE_MODE_UNRESTRICTED_PLUS); break;
			default: return;
		}
	} else if (assist_code == PAS_LEVEL_WALK) {
		app_ebike_set_mode(EBIKE_MODE_LEGAL);
	}
}
